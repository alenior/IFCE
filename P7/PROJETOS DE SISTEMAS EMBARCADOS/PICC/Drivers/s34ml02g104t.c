...
// Change History                                                            //
//                                                                           //
//    03-27-18 - Fixed a roll over issue with the flash_IncAddressPage()     //
//               function when the increment value plus the existing page    //
//               address was larger then 255.                                //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
FLASH_ERR flash_IncAddressPage(FLASH_ADDR *Addr, uint16_t value)
{
   ...
   memcpy(&Address, Addr, sizeof(FLASH_ADDR));
   
   while(value >= FLASH_BLOCK_PAGES)
   {
      if(++Address.Block >= FLASH_PLANE_BLOCKS)
      {
         if(++Address.Plane == 0)
      ...
      value -= FLASH_BLOCK_PAGES;
   }
   
   Address.Page += value;
   
   if(Address.Page >= FLASH_BLOCK_PAGES)
   {      
      if(++Address.Block >= FLASH_PLANE_BLOCKS)
      {
         if(++Address.Plane == 0)
      ...
      Address.Page -= FLASH_BLOCK_PAGES;
   }
   
   memcpy(Addr, &Address, sizeof(FLASH_ADDR));
...
FLASH_ERR flash_DecAddressPage(FLASH_ADDR *Addr)
{
   ...
   if(--Address.Page >= FLASH_BLOCK_PAGES)
   {
      if(--Address.Block >= FLASH_PLANE_BLOCKS)
      {
         ...
         Address.Block = FLASH_PLANE_BLOCKS - 1;
      }
      
      Address.Page = FLASH_BLOCK_PAGES - 1;
   }
...
FLASH_ERR flash_IncAddressColumn(FLASH_ADDR *Addr, uint16_t value)
{
   ...
   if(Address.Column >= FLASH_PAGE_WORDS)
   {
      if(++Address.Page >= FLASH_BLOCK_PAGES)
      {
         if(++Address.Block >= FLASH_PLANE_BLOCKS)
      ...
      Address.Column -= FLASH_PAGE_WORDS;
   }
...
FLASH_ERR flash_IncAddressBlock(FLASH_ADDR *Addr, uint16_t value)
{
   ...
   while(value >= FLASH_PLANE_BLOCKS)
   {
      if(++Address.Plane == 0)
         return(FLASH_ERR_FULL);
      
      value -= FLASH_PLANE_BLOCKS;
   }
   ...
   if(Address.Block >= FLASH_PLANE_BLOCKS)
   {
      ...
      Address.Block -= FLASH_PLANE_BLOCKS;
   }
...
int1 flash_IncAddress(FLASH_ADDR *Addr, uint16_t value)
{
   ...
   Error = flash_IncAddressColumn(Addr, value);
   
   if(Error == FLASH_ERR_OK)   
      return(TRUE);
   else
      return(FALSE);
}
