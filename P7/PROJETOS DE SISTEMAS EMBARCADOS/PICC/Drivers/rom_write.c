...
//// to properly handle modfying (so it will read the block of         ////
...
//// memory).  Also all erase/writes are delayed until a page boundry  ////
...
////                                                                   ////
//// January 26th, 2017:                                               ////
////  Won't attempt to write past end of flash memory.                 ////
////                                                                   ////
///////////////////////////////////////////////////////////////////////////
...
// total size of program memory of this controller, including config bits.  this value is in instructions (NOT BYTES)
   ...
   #define _ROMW_HAS_CONFIG_BITS 1
      ...
      #define _ROMW_WRITABLE_SIZE   _ROMW_FLASH_SIZE
   #else
      #define _ROMW_WRITABLE_SIZE   _ROMW_FIND_PAGE_START(getenv("PROGRAM_MEMORY"))
   #endif
#else
...
#define _ROMW_CONFIG_BITS_LOC    getenv("PROGRAM_MEMORY")
   ...
   void rom_modify(unsigned int32 location, unsigned char *src, unsigned int16 size)
   {
      ...
      while (size)
      {
         ...
         {
            debug_rom_write(debug_putc, "rom_modify() max=%lu, _ROMW_CONFIG_BITS_SIZE=%lu\r\n", max, _ROMW_CONFIG_BITS_SIZE);
         
            if (max < (unsigned int16)_ROMW_CONFIG_BITS_SIZE)
               max = 0;
            else
               max -= _ROMW_CONFIG_BITS_SIZE;
