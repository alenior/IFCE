...
// Acceptance Filter n Standard Identifier
...
// Acceptance Filter n Extended Identifier High
...
// Acceptance Filter n Extended Indenter Low
...
struct CiCFG2_struct {
   ...
   int seg2ph:3;     //8:10   phase buffer segment 2 bits
...
struct CiINTF_struct {
   ...
   int1 tx0if;       //2      transmit buffer 0 interrupt flag bit
   int1 tx1if;       //3      transmit buffer 1 interrupt flag bit
   int1 tx2if;       //4      transmit buffer 2 interrupt flag bit
...
struct rx_stat {
   uint8_t filthit;      // filler that allowed the frame into the buffer
