/*
   UART1.h
   
   Library for sending/receiving data on the PIC's UART 1 peripheral.
   
   For CCS C Compiler:
   This is mostly a wrapper to the #use rs232() serial library that is 
   built-in to the compiler.  If you are using a receive
   buffer you have to enable the global interrupts, the UART's peripheral
   interrupt enable bit will be controlled by this library.
   
   For XC8 compiler:
   plib is used which is only compatible with PIC18.
   plib it requires GetPeripheralClock() to be defined which is
   the peripheral clock (which is the same as the instruction clock,
   or system clock divided by 4).
   If you are using a receive buffer, you will have to create an ISR
   and in the ISR call UART1RxIsrTask().  If you are using a receive
   buffer you have to enable the global interrupts, the UART's peripheral
   interrupt enable bit will be controlled by this library.
   
   For XC16/XC32 compiler:
   No external library is used; this library is done by accessing the UART
   peripheral directly.  If you are using a receive buffer, nothing else
   needs to be done (everything is handled by this library).
   
   CONFIGURATION:
   -------------------------------------------------------------------
   UART1_RX_BUFFER_SIZE
   If you want to use an ISR receive buffer, define this value to the size
   (in bytes) of the buffer you want to use.  On 8-bit PICmicros (PIC16,
   PIC18, etc) the library will turn on the appropiate peripheral interrupt 
   enable bit, but the user is responsible for turning on the global interrupt 
   enable bit (enable_interrupts(GLOBAL) in CCS C).
*/
///////////////////////////////////////////////////////////////////////////
////        (C) Copyright 1996,2015 Custom Computer Services           ////
////                                                                   ////
//// This source code may only be used by licensed users of the CCS C  ////
//// compiler or to users who purchased the rights to use this code.   ////
//// This source code may only be distributed to other licensed users  ////
//// of the CCS C compiler or other users who purchased the rights to  ////
//// this code.  The rights to use this code can only be granted       ////
//// by CCS.  No other use, reproduction or distribution is permitted  ////
//// without written permission.  Derivative programs created using    ////
//// this software in object code form are not restricted in any way.  ////
////                                                                   ////
////                     http://www.ccsinfo.com                        ////
///////////////////////////////////////////////////////////////////////////

#ifndef __UART1_H__
#define __UART1_H__

#if defined(__XC8) || defined(__XC16) || defined(__XC32)
   #include <system.h>
#endif

#include <stdint.h>
#include <stdbool.h>

#if !defined(UART1_RX_BUFFER_SIZE)
   #define UART1_RX_BUFFER_SIZE 0
#endif

#if (UART1_RX_BUFFER_SIZE == 0)
   typedef bool uart1_rx_size_t;
#elif (UART1_RX_BUFFER_SIZE >= 0x100)
   typedef uint16_t uart1_rx_size_t;
#else
   typedef uint8_t uart1_rx_size_t;
#endif

// Initialze and turn on the UART.
// If using RX buffering, will turn on the RX interrupt, but it WILL NOT turn
// on the global interrupt enable bit.
// Using this function on CCS C Compiler you are limited to these baud rates:
//    300,1200,2400,4800,9600,19200,38400,57600,115200
void UART1Init(uint32_t baud);

// Send a character to the UART.
void UART1Putc(char c);

// Get a character from the UART.  Will wait for data to be ready for RX,
// which can cause a lockup condition if no data is being received.
char UART1Getc(void);

// if receive buffering was enabled, this returns the number of characters 
// in the receive buffer than can be read with UART1Getc().
// if receive buffering was not enabled, returns TRUE if there is data 
// to be read from the UART peripheral.
uart1_rx_size_t UART1Kbhit(void);

// Put a string (a null terminated array of chars) to the UART, does not send
// the null terminator.
void UART1Puts(char *p);
void UART1PutsROM(rom char *p);

#if (defined(__XC8) && (UART1_RX_BUFFER_SIZE > 0))
// If using a receive buffer and XC8 compiler, then you will need to manually
// add the ISR function in your project and call 
inline void UART1RxIsrTask(void);
#endif

#endif   //ifndef __UART1_H__