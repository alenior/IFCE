...
////   It is strongly suggested you use the j1939.c driver instead.  The   ////
////   j1939.c driver uses new can drivers with a new API, additionally    ////
////   support for devices with the new CANFD peripheral was added.        ////
////                                                                       ////
///////////////////////////////////////////////////////////////////////////////
...
void J1939Init(void)
{
   ...
   can_init();    //Initialize the CAN, sets up Baud Rate and puts it in normal mode
      ...
      can_set_id((uint8_t *)RX0MASK, 0x0000FF00, CAN_USE_EXTENDED_ID);       //Set Mask 0 to look at Destination Address of PDU
      can_set_id((uint8_t *)RX1MASK, 0x00F00000, CAN_USE_EXTENDED_ID);       //Set Mask 1 to look at upper nibble of PDU Format
      
      can_set_id((uint8_t *)RXFILTER0, 0x0000FF00, CAN_USE_EXTENDED_ID);     //Filter 0 set to look for messages to the Global Address 255
      can_set_id((uint8_t *)RXFILTER1, 0x0000FF00, CAN_USE_EXTENDED_ID);     //Filter 1 set to look for messages to the Global Address 255 will change to unit's address once it gets one
      can_set_id((uint8_t *)RXFILTER2, 0x00F00000, CAN_USE_EXTENDED_ID);     //Filter 2 set to look for Broadcast messages PDU 240 to 255
...
void J1939SetCANFilter(uint8_t address)
{
   can_set_mode(CAN_OP_CONFIG);  //put CAN in Config mode
    ...
    #else
      can_set_id((uint8_t *)RXFILTER1, (uint32_t)address << 8, CAN_USE_EXTENDED_ID);     //Set Filter 1
    #endif
   #else
