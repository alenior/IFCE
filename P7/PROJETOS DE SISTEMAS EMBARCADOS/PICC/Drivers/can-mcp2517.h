...
/*
   ...
   CAN_RX_BUFFERS
      ...
      0-32, 0 disables it.  Default size if 16.
   ...
   CAN_TIME_BASE_COUNTER_PRESCALAR
      ...
      20MHz and a divisor of 20, the time base counter will increment once for
      every 20 CAN Clock ticks - 1/(20MHz / 20) = 1us rate.  Default is 1.
   ...
   MCP2517_EXT_CLOCK_SPEED
      Specifies the speed of the external crystal connected to the MCP2517FD.
      Only 4MHz, 20MHz and 40MHz crystals are supported.  Default us 20000000
      (20 MHz).
   
   MCP2517_SYSTEM_CLOCK_DIVISOR
      Sets the system clock divisor, 1 or 2.  Default is 1.
   
   MCP2517_ENABLE_CLOCK_OUTPUT
      Enables or disables the output clock, which can be used as clock source
      for other devices.  Default is FALSE, disabled.
   
   MCP2517_CLOCK_OUT_DIVISOR
      Sets the output clock divisor, 1, 2, 4 or 10.  Default is 1.
   
   OPTIONAL OBJECT PREPROCESSORS:
   ----------------------------------------------------------------------------
   
   CAN_USE_FIFO_x_OBJECT
      Used to enable FIFO2 - FIFO31, or FIFO1 if CAN_RX_BUFFERS is defined to 0.
      x being the FIFO to enable (1-31).
   ...
   CAN_FIFO_x_OBJECT_TX_RETRANSMISSION_ATTEMPTS
      ...
      available settings.  x being the FIFO to setup (1-31).
   ...
   CAN_FIFO_x_OBJECT_RX_TIME_STAMP
      ...
      enabled.  x being the FIFO to setup (1-31).
   ...
   CAN_USER_FILT_x
      Used to set the Id of the filter, x being the filter (0-31) to assign
      the Id to.
   
   CAN_USER_MASK_x
      Used to set the Mask of the filter, x being the filter (0-31) to assign
      the Mask to.
   
   CAN_USER_FILT_x_TYPE
      Used to set the filter type, x being the filter (0-31) to assign the type
   ...
   CAN_USER_FILT_x_OBJECT
      ...
      (0-31) to assign to object.  See CAN_OBJECT for object that filter can be
      assigned to.
   ...
   MCP2517 SPI PREPROCESSORS
   ----------------------------------------------------------------------------
   
   MCP2517Xfer
      Optional used to specify the SPI XFER function to use for communicating
      with the MCP2517FD device.  Not defined by default.
   
   MCP2517_SPI_STREAM
      Optional used to specify the #use spi() stream to use for communication
      with the MCP2517FD device.  If not defined before this driver is
      included, it will be defined to the #use spi() stream added by this
      driver.
   
   MCP2517_SPI_SCK_PIN
      Used to specify the SPI clock pin.  Only need if both MCP2517Xfer and
      MCP2517_SPI_STREAM are not defined before this driver is included.  In
      that case the default is PIN_C3.
   
   MCP2517_SPI_SDI_PIN
      Used to specify the SPI slave DI pin.  Only need if both MCP2517Xfer and
      MCP2517_SPI_STREAM are not defined before this driver is included.  In
      that case the default is PIN_C5.
   
   MCP2517_SPI_SDO_PIN
      Used to specify the SPI slave DO pin.  Only need if both MCP2517Xfer and
      MCP2517_SPI_STREAM are not defined before this driver is included.  In
      that case the default is PIN_C4.
   
   MCP2517_SPI_CS_PIN
      Used to specify the SPI Chip select pin, the default is PIN_B0.
   
   MCP2517_SPI_BAUD
      Used to specify the SPI baud rate to used. Only need if both MCP2517Xfer
      and MCP2517_SPI_STREAM are not defined before this driver is included.
      In that case the default value depends on system clock speed, 20MHz for
      40MHz, 10MHz for 20MHz or 5MHz for 10MHz.
   
   MCP2517_SPI_USE_CRC
      Used to specify whether to use CRC for all SPI communication with
      MCP2517FD device, the default is FALSE, not used.
   
   MCP2517_SPI_FORCE_SW_CRC
      If MCP2517_SPI_USE_CRC is defined to TRUE and PIC has hardware CRC
      peripheral, it is used to specify using SW CRC instead of HW CRC.  If PIC
      has a CRC peripheral and MCP2517_SPI_USE_CRC is defined to TRUE, it 
      defaults to TRUE, use HW CRC peripheral.  If PIC doesn't have CRC 
      peripheral and MCP2517_SPI_USE_CRC is defined to TRUE, it is forced to
      TRUE.
   
   MCP2517_SPI_USE_LOOKUP_TABLE
      If MCP2517_SPI_USE_CRC is defined to TRUE and MCP2517_SPI_FORCE_SW_CRC is
      defined to TRUE, used to specify using a lookup table to calculate the
      CRC.  The default is TRUE, use lookup table.
   
   MCP2517_SPI_USE_RAM_LOOKUP_TABLE
      If MCP2517_SPI_USE_CRC is defined to TRUE, MCP2517_SPI_FORCE_SW_CRC is
      defined to TRUE and MCP2517_SPI_USE_LOOKUP_TABLE is defined to TRUE, used
      to specify using a RAM based lookup table.  The default is FALSE, use ROM
      based table.
   
   MCP2517_SPI_DO_SAFE_WRITE
      If MCP2517_SPI_USE_CRC is defined to TRUE, used to specify using CRC safe
      write commands for writing MCP2517FD SFRs and Message memory.  The default
      is FALSE, don't use.
   
   MCP2517_INT_PIN
      Optional used to specify the PIC pin connected to the MCP2517FD INT pin.
      The INT pin is asserted on any interrupt in the CiINT register.
   
   MCP2517_INT0_PIN
      Optional pin specifying the PIC pin connected to the MCP2517FD INT0/GPIO0
      pin.  If defined this driver will setup the INT0/GPIO0 pin as an
      interrupt pin that is asserted when a TX interrupt occurs.
   
   MCP2517_INT1_PIN
      Optional pin specifying the PIC pin connected to the MCP2517FD INT1/GPIO1
      pin.  If defined this driver will setup the INT1/GPIO1 pin as an
      interrupt pin that is asserted when a RX interrupt occurs.
   
   MCP2517_INT_OUTPUT_MODE
      Sets the output mode of the INT, INT0 and INT1 pins, 0 - Push/Pull, 1 - 
      Open Drain.  The default is 1, Open Drain.
   
   MCP2517_TXCAN_OUTPUT_MODE
      Sets the output mode of the TXCAN pin, 0 - Push/Pull, 1 - Open Drain.
      The default is 0, Push/Pull.
   
   MCP2517_CLKO_TYPE
      Sets what is outputted on the CLKO pin, 0 - Clock, 1 - SOF signal.  The
      defaults is 0, Clock.
   
   MCP2517_XSTBY
      If MCP2517_INT0_PIN not defined, used to enable/disable XSTBY control
      with the INT0/GPIO0 pin.  The defaults is 0, disabled.
   
   MCP2517_TRIS0
      If MCP2517_INT0_PIN not defined and MCP2517_XSTBY defined as 0, used to
      set the data direction of the INT0/GPIO0 pin, 0 - output, 1 - input.  The
      default is 1, input.
   
   MCP2517_DEFAULT_LAT0
      If MCP2517_INT0_PIN not defined, MCP2517_XSTBY defined as 0 and
      MCP2517_TRIS0 defined as 0, sets the default output level of the
      INT0/GPIO0 pin when MCP2517FD is initialized.  The default is 0, low.
   
   MCP2517_TRIS1
      If MCP2517_INT1_PIN not defined, used to set the data direction of the
      INT1/GPIO1 pin, 0 - output, 1 - input.  The default is 1, input.
   
   MCP2517_DEFAULT_LAT1
      If MCP2517_INT1_PIN not defined and MCP2517_TRIS1 defined as 0, sets the
      default output level of the INT1/GPIO1 pin when MCP2517FD is initialized.
      The default is 0, low.
...
#ifndef __CAN_MCP2517_H__
#define __CAN_MCP2517_H__
...
typedef enum
{
   ...
   CAN_EC_SPI_CRC,               //SPI read or write failed CRC check
} can_ec_t;
...
// CAN Filter type
typedef enum {CAN_FILTER_0, CAN_FILTER_1, CAN_FILTER_2, CAN_FILTER_3, CAN_FILTER_4, CAN_FILTER_5, CAN_FILTER_6, CAN_FILTER_7,
              CAN_FILTER_8, CAN_FILTER_9, CAN_FILTER_10, CAN_FILTER_11, CAN_FILTER_12, CAN_FILTER_13, CAN_FILTER_14, CAN_FILTER_15,
              CAN_FILTER_16, CAN_FILTER_17, CAN_FILTER_18, CAN_FILTER_19, CAN_FILTER_20, CAN_FILTER_21, CAN_FILTER_22, CAN_FILTER_23,
              CAN_FILTER_24, CAN_FILTER_25, CAN_FILTER_26, CAN_FILTER_27, CAN_FILTER_28, CAN_FILTER_29, CAN_FILTER_30, CAN_FILTER_31
} CAN_FILTER;
...
// CAN Object type.
typedef enum {CAN_OBJECT_TXQ, CAN_OBJECT_FIFO_1, CAN_OBJECT_FIFO_2, CAN_OBJECT_FIFO_3, CAN_OBJECT_FIFO_4, CAN_OBJECT_FIFO_5, CAN_OBJECT_FIFO_6, CAN_OBJECT_FIFO_7,
              CAN_OBJECT_FIFO_8, CAN_OBJECT_FIFO_9, CAN_OBJECT_FIFO_10, CAN_OBJECT_FIFO_11, CAN_OBJECT_FIFO_12, CAN_OBJECT_FIFO_13, CAN_OBJECT_FIFO_14, CAN_OBJECT_FIFO_15, 
              CAN_OBJECT_FIFO_16, CAN_OBJECT_FIFO_17, CAN_OBJECT_FIFO_18, CAN_OBJECT_FIFO_19, CAN_OBJECT_FIFO_20, CAN_OBJECT_FIFO_21, CAN_OBJECT_FIFO_22, CAN_OBJECT_FIFO_23,
              CAN_OBJECT_FIFO_24, CAN_OBJECT_FIFO_25, CAN_OBJECT_FIFO_26, CAN_OBJECT_FIFO_27, CAN_OBJECT_FIFO_28, CAN_OBJECT_FIFO_29, CAN_OBJECT_FIFO_30, CAN_OBJECT_FIFO_31
} CAN_OBJECT;
...
typedef struct
{
   ...
   uint8_t Length;            //Number of data bytes to send, for lengths larger then 8 bytes see DATA_LENGTH type above for valid options
...
typedef enum
{
   ...
   CAN_INTERRUPT_ECC = 0x0100,         //ECC Error Interrupt
   CAN_INTERRUPT_SPI_CRC=0x0200,       //SPI CRC Error Interrupt
