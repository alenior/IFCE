////////////////////////////////////////////////////////////////////////////////
////                                                                        ////
////   This driver is deprecated and provided for backwards compatibility.  ////
////   It is strongly suggested you use the can-pic24_dsPIC33.c driver      ////
////   instead, which use a new API.                                        ////
////                                                                        ////
////////////////////////////////////////////////////////////////////////////////
...
////                                                                        ////
////  can_set_fifo_start_buffer() Sets the start buffer of the FIFO buffer  ////
////     for CAN1.                                                          ////
////  can2_set_fifo_start_buffer() Sets the start buffer of the FIFO buffer ////
////     for CAN2.                                                          ////
////                                                                        ////
...
//// Version History                                                        ////
////                                                                        ////
////     Jan 31 19 - Fixed an issue with the can_set_buffer_id() and        ////
////                 can2_set_buffer_id() functions that could cause the    ////
////                 RTR bit to be set in the message buffer, which causes  ////
////                 some devices to transmit incorrect data when that      ////
////                 buffer is setup to respond to an RTR request.          ////
////                                                                        ////
////     Jan 25 19 - Fixed an issue with the can_load_rtr() and             ////
////                 can2_load_rtr() functions not loading data correctly   ////
////                 which caused incorrect data and ID to be sent in some  ////
////                 cases.  Also fixed an issue with can_fifo_getd() and   ////
////                 can2_fifo_getd() function not receiving entire message ////
////                 correctly.                                             ////
////                                                                        ////
////     Oct 22 18 - Changed how can_fifo_getd() and can2_fifo_getd()       ////
////                 checks buffers for messages.  To fix issue with        ////
////                 receiving multiple messages between calls causing it   ////
////                 to get behind, and to fix issue when one or more       ////
////                 message buffers have been overflown.                   ////
////                                                                        ////
////                 Added can_set_fifo_start_buffer() and                  ////
////                 can2_set_fifo_start_buffer() functions.                ////
////                                                                        ////
////     Mar 21 17 - Added DONT_USE_ERRATA_WORKAROUND option to build code  ////
////                 without work around implemented for dsPIC33EV family.  ////
////                                                                        ////
////     May 02 16 - Added work around for dsPIC33EV family.                ////
////                                                                        ////
////////////////////////////////////////////////////////////////////////////////
...
#define can_tx_is_empty() ((!C1TR01CON.txenm || !C1TR01CON.txreqm) && (!C1TR01CON.txenn || !C1TR01CON.txreqn) && (!C1TR23CON.txenm || !C1TR23CON.txreqm) && (!C1TR23CON.txenn || !C1TR23CON.txreqn) && (!C1TR45CON.txenm || !C1TR45CON.txreqm) && (!C1TR45CON.txenn || !C1TR45CON.txreqn) && (!C1TR67CON.txenm || !C1TR67CON.txreqm) && (!C1TR67CON.txenn || !C1TR67CON.txreqn))
...
// Buffer for CAN2
   ...
   uint8_t ecan2_last_fifo_buffer = CAN_BUFFER_SIZE;
...
//
////////////////////////////////////////////////////////////////////////////////
void can_set_fifo_start_buffer(uint8_t buffer)
{
   if(buffer < 32)
      C1FCTRL.fsa = buffer;
}
...
#if getenv("SFR_VALID:C2CTRL1")
   void can2_set_fifo_start_buffer(uint8_t buffer)
   {
      if(buffer < 32)
         C1FCTRL.fsa = buffer;
   }
...
int1 can_putd(uint32_t id, uint8_t *data, uint8_t len, uint8_t priority, int1 ext, int1 rtr) 
{
   ...
   if(ext)
   {
      temp=(((id & 0x1FFC0000) >> 16) | 0x0003) & 0x1FFF;
   ...
   {  
      temp=(((id & 0x7FF) << 2) | ((uint16_t)rtr << 1)) & 0x1FFF;
      ecan1_message_buffer[port][0]=temp;
      ecan1_message_buffer[port][1]=0x0000;
      temp = ((uint16_t)len) & 0x000F;
      ecan1_message_buffer[port][2]=temp;
   }
...
int1 can_fifo_getd(uint32_t &id, uint8_t *data, uint8_t &len, struct rx_stat &stat )
{
   ...
   buffer = C1FIFO.fnrb;
   rxful1 = (uint16_t)C1RXFUL1;
   rxful2 = (uint16_t)C1RXFUL2;
   ...
   do
   {
      if(buffer < 16)
      {
         if(bit_test(rxful1, buffer) == TRUE)
            done = TRUE;
         else
         {
            if(++buffer > ecan1_last_fifo_buffer)
               buffer = (uint16_t)C1FCTRL.fsa;
         }
      }
      else
      {
         if(bit_test(rxful2, (buffer - 16)) == TRUE)
            done = TRUE;
         else
         {
            if(++buffer > ecan1_last_fifo_buffer)
   ...
   stat.buffer=buffer;
   memcpy(temp_buffer, &ecan1_message_buffer[buffer][0], 16);
   
  #ifndef DONT_USE_ERRATA_WORKAROUND
   //This change was made to workaround an errata the dsPIC33EV family has,
   //implemented as recommended in Microchip's errata documentation.
   if(buffer < 16)
   {
      stat.err_ovfl = bit_test(C1RXOVF1, buffer);
   
      (uint16_t)C1RXFUL1 = ~(0x0001 << buffer);
      (uint16_t)C1RXOVF1 &= ~(0x0001 << buffer);
   }
   else
   {
      buffer -= 16;
      
      stat.err_ovfl = bit_test(C1RXOVF2, buffer);
   
      (uint16_t)C1RXFUL2 = ~(0x0001 << buffer);
      (uint16_t)C1RXOVF2 &= ~(0x0001 << buffer);
   }
  #else
   if(buffer < 16)
      stat.err_ovfl = bit_test(C1RXOVF1, buffer);
   ...
   //Only clear rbif if both C1RXFUL1 and C1RXFUL2 are zero, indicating that there
   //are no more messages to receive.
   if(((uint16_t)C1RXFUL1 == 0) && ((uint16_t)C1RXFUL2 == 0))
      C1INTF.rbif = 0;
   ...
   int1 can2_fifo_getd(uint32_t &id, uint8_t *data, uint8_t &len, struct rx_stat &stat )
   {
      ...
      buffer = C2FIFO.fnrb;
      rxful1 = (uint16_t)C2RXFUL1;
      rxful2 = (uint16_t)C2RXFUL2;
      ...
      do
      {
         if(buffer < 16)
         {
            ...
            {
               if(++buffer > ecan2_last_fifo_buffer)
                  buffer = (uint16_t)C2FCTRL.fsa;
            }
         }
         else
         {
      ...
      stat.buffer = buffer;
      memcpy(temp_buffer, &ecan2_message_buffer[buffer][0], 16);
      ...
      if(stat.buffer < 16)
      {
         stat.err_ovfl = bit_test(C2RXOVF1, buffer);
         
         (uint16_t)C2RXFUL1 = ~(0x0001 << buffer);
         (uint16_t)C2RXOVF1 &= ~(0x0001 << buffer);
      }
      else
      {
         buffer -= 16;
         
         stat.err_ovfl = bit_test(C2RXOVF2, buffer);
         
         (uint16_t)C2RXFUL2 = ~(0x0001 << buffer);
         (uint16_t)C2RXOVF2 &= ~(0x0001 << buffer);
      }
     #else
      if(buffer < 16)
         stat.err_ovfl = bit_test(C2RXOVF1, buffer);         
      ...
      //Only clear rbif if both C2RXFUL1 and C2RXFUL2 are zero, indicating that there
      //are no more messages to receive.
      if(((uint16_t)C2RXFUL1 == 0) && ((uint16_t)C2RXFUL2 == 0))
         C2INTF.rbif = 0;
