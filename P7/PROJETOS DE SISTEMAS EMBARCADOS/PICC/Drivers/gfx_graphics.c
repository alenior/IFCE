...
//                                                                           //
//    HANDLE_ID gfx_DisplayImage(uint16_t Index, uint16_t x, uint16_t y)     //
...
//                                                                           //
//    void gfx_RemoveImage(HANDLE_ID HandleId, int1 RedrawScreen = FALSE)    //
...
//                                                                           //
//    void gfx_FillArea(HANDLE_ID ImageHandle, uint8_t AreaIndex,            //
...
//          filled in.                                                       //
//                                                                           //
//    void gfx_DrawRectangleAbsolute(uint16_t StartX, uint16_t StartY,       //
//                                   uint16_t StopX, uint16_t StopY,         //
//                                   uint16_t Color, int1 Fill=FALSE);       //
//       - Draw a rectangle with the specified color starting at coordinates //
//         (StartX,StartY) and ending at coordinates (StopX,StopY).  If Fill //
//         is TRUE the rectangle will be filled in.                          //
//                                                                           //
//    void gfx_DisplayString(HANDLE_ID ImageHandle, uint8_t AreaIndex,       //
//                           uint8_t FontIndex, uint16_t Fcolor,             //
//                           uint16_t Bcolor, char *Text, uint8_t Mode=0)    //
...
//         designated as AREA_TYPE_TEXT.  Mode is an optional parameter used //
//         to set special features of the text, e.g. text alignment in area: //
//           * Bit 0 - 1 = Center Horizontally, 0 = Left Aligned             //
//           * Bit 1 - 1 = Center Vertically, 0 = Top Aligned                //
//           * Bit 2 - 1 = Transparent Background, 0 = Display Background    //
//                                                                           //
...
//         (x,y).  Mode is an optional parameter used to set special         //
//         features of the text, e.g. transparent background:                //
//           * Bit 2 - 1 = Transparent Background, 0 = Display Background    //
//                                                                           //
//    uint16_t gfx_GetImageIndex(HANDLE_ID HandleId)                         //
//       - Gets the Image Index from the specified Handle Id.                //
//                                                                           //
//    HANDLE_ID gfx_GetImageHandle(uint16_t ImageIndex)                      //
//       - Get the Handle Id from the specified Image Index.                 //
//                                                                           //
// Defines:                                                                  //
//                                                                           //
//    TAB_CHARACTERS                                                         //
...
//       HANDLE_ID HandleId;   - The HandleId assigned to the image.         //
...
//                                                                           //
//    IMAGE_ERR gfx_LoadImage(uint16_t Count, uint8_t *Data, int1 New)       //
...
//         device.                                                           //
//                                                                           //
//    FONT_ERR gfx_LoadFont(uint16_t Count, uint8_t *Data, int1 New)         //
...
//                                                                           //
//    TOUCH_RELEASE_RATE                                                     //
//       - Specifies the time in milliseconds after the display as been      //
//         released to call the user's touch function by gfx_TouchTask()     //
//         with the kind set to TOUCH_TYPE_RELEASED.                         //
//                                                                           //
// Types:                                                                    //
//                                                                           //
//    TOUCH_TYPE:                                                            //
...
//         to generate the PWM signal to turn on, turn off and dim the       //
//         backlight.  Pin must be one of the hardware Output Compare/CCP    //
//         pins.  The above backlight functions and macros are only          //
//         available if GFX_BACKLIGHT_PIN is defined.                        //
//                                                                           //
//    GFX_BACKLIGHT_TIMER                                                    //
...
//                                                                           //
//    glcd_EnableTransparency(uint16_t Color)                                //
//                                                                           //
//    glcd_DisableTransparency(void)                                         //
//                                                                           //
...
//                                                                           //
//    GFX_TICKS_PER_SECOND                                                   //
//       - The number of ticks that the tick timer ticks per second.         //
//                                                                           //
//    gfx_get_tick                                                           //
//       - Macro to return the current tick timer value.                     //
//                                                                           //
//    gfx_tick_difference                                                    //
//       - Macro to return the difference between to tick values.            //
//                                                                           //
// Types:                                                                    //
//                                                                           //
//    GFX_TICK                                                               //
//       - The variable type used for the tick timer.                        //
//                                                                           //
//    FLASH_ADDR                                                             //
//       - The variable type used for addressing external memory.            //
//                                                                           //
...
//                        uint16_t StartY)                                   //
//                                                                           //
// The following function is required if the define USE_FAST_VERTICAL_DRAW   //
// is used in addition to the USE_FAST_DRAW function:                        //
//                                                                           //
//    void WriteFastVerticalImage(IMAGE *ImageData, uint16_t StartX,         //
//                                uint16_t StartY)                           //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// Aug 07 2019  - Updated gfx_TouchTask() function so if TOUCH_UPDATE_RATE   //
//                is defined to 0 and OnlyUpDown parameter is set to FALSE   //
//                with gfx_SetTouchCallback() function, TouchCallback()      //
//                function will be called at rate touch hardware has new     //
//                touch coordinates ready to read.  This change was made to  //
//                aid in developing debounce code for touches.               //
//                                                                           //
//                Updated TouchCallback() function to always call user's     //
//                touch function when kind is TOUCH_TYPE_RELEASED.  This     //
//                change was made to aid in developing debounce code for     //
//                touches.                                                   //
//                                                                           //
//                Added TOUCH_RELEASE_RATE define to driver code.  This was  //
//                added to sperate it from the TOUCH_UPDATE_RATE define,     //
//                for cases were TOUCH_UPDATE_RATE is defined as 0.          //
//                                                                           //
// Apr 29 2019  - Fixed an issue with a change in how compiler handles       //
//                signed and unsigned values that was causing some of the    //
//                code in TouchTask() to be compiled out.                    //
//                                                                           //
// Sept 28 2017 - Fixed an issue with extra new line being added with        //
//                gfx_DisplayString() and gfx_DisplayStringAbsolute()        //
//                functions in certain cases.                                //
//                                                                           //
//                Added USE_FAST_VERTICAL_DRAW option, when USE_FAST_DRAW    //
//                and USE_FAST_VERTICAL_DRAW are both defined the            //
//                gfx_DisplayImage() function will call                      //
//                WriteFastVerticalImage() for displaying an image when      //
//                vertical orientation is enabled.  WriteFastVerticalImage() //
//                is a user provided function which reads an image out of    //
//                the external memory that has been pre-rotated 90 degrees   //
//                counter clockwise.  The header of the pre-rotated images   //
//                should still have the original orientation listed for the  //
//                image's width and height.  For example if the display is   //
//                480x272, a full screen vertical orientated image's width   //
//                is 272 and it's height it 480.  The image's header should  //
//                have the width as 272 and height as 480, but the pixel     //
//                data is organized as top to bottom, right to left instead  //
//                of left to right, top to bottom.  CCS's Interface Designer //
//                software as been updated to download images that support   //
//                the USE_FAST_VERTICAL_DRAW option.  This can be enabled by //
//                going to the Settings Tab and checking the 'Vertical'      //
//                check box in the Orientation section followed by checking  //
//                the 'Fast Vertical Draw' check box.  Then going to the     //
//                Image Tab and using the 'Load' button to download the      //
//                images to the board.                                       //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
//
// Returns:
//    HANDLE_ID value representing the Handle Id assigned to the image.  If it 
...
//
///////////////////////////////////////////////////////////////////////////////
void gfx_FillArea(HANDLE_ID ImageHandle, uint8_t AreaIndex, FILL_DIR Direction, uint32_t Pixels, uint16_t Color)
{
...
//
///////////////////////////////////////////////////////////////////////////////
void gfx_DrawRectangleAbsolute(uint16_t StartX, uint16_t StartY, uint16_t StopX, uint16_t StopY, uint16_t Color, int1 Fill=FALSE)
{
   DrawRectangle(StartX, StartY, StopX, StopY, Color, Fill);   
}
...
//
//    Mode - optional parameter used to set special features of the text, e.g.
//           text alignment in area.
//             * Bit 0 - 1 = Center Horizontally, 0 = Left Aligned
//             * Bit 1 - 1 = Center Vertically, 0 = Top Aligned
//             * Bit 2 - 1 = Transparent Background, 0 = Display Background
//
///////////////////////////////////////////////////////////////////////////////
void gfx_DisplayString(HANDLE_ID ImageHandle, uint8_t AreaIndex, uint8_t FontIndex, uint16_t Fcolor, uint16_t Bcolor, char *Text, uint8_t Mode = 0)
{
   uint8_t numberOfNewLines=1;
   HANDLE_STRUCT Handle;
   AREA_INFO Area;
   uint16_t StartX, StartY, TextboxStartX, TextboxStartY;
   uint16_t EndX, EndY, TextboxEndX, TextboxEndY;
   ...
   char*TempText1;
   TempText=Text;
   TempText1=Text;//reset pointer
   uint16_t textHeight;
   int stringSizeCounter=0;//counts size of a new line
//!   int stringBool=1;//used to count #of chars only once per new line
   int firstTime=1;//used to allow the centering of the first line
   ...
   if(GetHandle(ImageHandle, &Handle))
   {
      if(GetArea(AreaIndex, &Handle, &Area))
      {
         if(Area.Type == AREA_TYPE_TEXT) //Make sure area is a Text Box
         {
            ...
            TextboxStartX = Handle.x + Area.x1;
            TextboxStartY = Handle.y + Area.y1;
            TextboxEndX = Handle.x + Area.x2 + 1;
            TextboxEndY = Handle.y + Area.y2 + 1;
            CurrentX = StartX;
            CurrentY = StartY;
            ...
            ///vertical centering///
            textHeight = numberOfNewLines*CharHeight;
            
            if(bit_test(Mode, 1))
            {
               if((numberOfNewLines % 2) > 0)
               {
                  CurrentY = TextboxStartY + (((TextboxEndY - TextboxStartY) / 2) - (CharHeight / 2)) - (((numberOfNewLines-1)/2)*CharHeight);
               }
               else if((numberOfNewLines % 2) == 0)
               {
                  CurrentY = TextboxStartY + (((TextboxEndY - TextboxStartY) / 2 ) - ((numberOfNewLines / 2) * CharHeight));
               }
               
               if(textHeight >= (TextboxEndY - TextboxStartY))
                  CurrentY = TextboxStartY;
            }
            
            if((CurrentX + CharWidth) <= TextboxEndX) //Make sure TextBox is wide enough to display at least one character
            {
               if(bit_test(Mode, 2))
                  glcd_EnableTransparency(Bcolor);
            
               while((*Text != '\0') && ((CurrentY) < TextboxEndY))
               {
                  if(firstTime && (*TempText1 != '\0'))
                  {
                     while((*TempText1 != '\n') && (*TempText1 != '\0'))
                     {
                        stringSizeCounter++;
                     ...
                     if(bit_test(Mode, 0))
                     {
                        if((stringSizeCounter % 2) != 0)
                        {
                           CurrentX = TextboxStartX + (((TextboxEndX - TextboxStartX) / 2) - (CharWidth / 2)) - (((stringSizeCounter - 1) / 2) * CharWidth);
                           
                           //The following should be taken care of above
//!                           if(bit_test(Mode, 1) && stringBool)
//!                           {
//!                              if(numberOfNewLines % 2 > 0)
//!                              {
//!                                 CurrentY = TextboxStartY + (((TextboxEndY - TextboxStartY) / 2) - (CharHeight / 2)) - (((numberOfNewLines - 1) / 2) * CharHeight);
...
//!
                        ...
                        {
                           CurrentX = TextboxStartX + (((TextboxEndX - TextboxStartX) / 2) - ((stringSizeCounter / 2) * CharWidth));
...
//!                              if(numberOfNewLines % 2 > 0)
//!                              {
//!                                 CurrentY = ((TextboxEndY / 2) - (CharHeight / 2)) - (((numberOfNewLines - 1) / 2) * CharHeight);
//!                              }
//!                              else if(numberOfNewLines % 2 == 0)
//!                              {
//!                                 CurrentY = ((TextboxEndY / 2) - ((numberOfNewLines / 2) * CharHeight));
//!                                 
//!                              }
//!                              
//!                              if(textHeight >= (TextboxEndY - TextboxStartY))
...
//!                           }
                        ...
                        if((stringSizeCounter * CharWidth) >= (TextboxEndX - TextboxStartX))
                           CurrentX = TextboxStartX;
                     }

                     TempText1++;
                     firstTime = 0;
                     stringSizeCounter = 0;//restart counter
                  }
                  ...
                  else if(*Text == '\n')
                  {
                     ...
                     CurrentY += CharHeight;
                     CurrentX = TextboxStartX;  //so it can't overflow with check below
                  }
                  else if(*Text == '\t')
                  ...
                  else
                  {
                     ...
                     ///Changes for too big of font being displayed///
                     if((CurrentY + CharHeight) >= EndY)
                     {
                        CharHeight = EndY - CurrentY;
                     }
                  ...
                  if(((CurrentX + CharWidth) > EndX) && (*Text != '\n') && (*Text != '\r'))
                  {
                     CurrentY += CharHeight;
                     CurrentX = StartX;
                  }
...
//
///////////////////////////////////////////////////////////////////////////////
void gfx_DisplayStringAbsolute(uint16_t x, uint16_t y, uint8_t FontIndex, uint16_t Fcolor, uint16_t Bcolor, char *Text, uint8_t Mode = 0)
{
   uint8_t *CharacterData;
   uint16_t *Pixels;
   int16_t i;
...
//
// Parameters:
//    HandleId - Handle Id to look up.
//
// Returns:
//    uint16_t of the Image Index of the Image with the specified Handle Id, -1
//    if the specified Handle Id doesn't exist.
//
///////////////////////////////////////////////////////////////////////////////
uint16_t gfx_GetImageIndex(HANDLE_ID HandleId)
{
   ...
   Result = GetHandle(HandleId, &Handle);
...
// gfx_GetImageHandle()
//
// Gets the Handle Id for the specified Image.
//
// Parameters:
//    ImageIndex - Image Index to look up.
//
// Returns:
//    HANDLE_ID of the Handle Id for the specified Image Index, -1 if the
//    specified Image doesn't have a Handle Id.
//
///////////////////////////////////////////////////////////////////////////////
HANDLE_ID gfx_GetImageHandle(uint16_t ImageIndex)
{
   ...
   for(i=0;i<gfx_HandleCount;i++)
   {
      if(gfx_ImageHandle[i].ImageIndex == ImageIndex)
      {
         result = gfx_ImageHandle[i].HandleId;
         break;
      }
   }
...
int1 WriteImage(uint16_t ImageIndex, uint16_t StartX, uint16_t StartY)
{
   IMAGE ImageData;
   IMAGE_ERR Error;
  #if !defined(USE_FAST_VERTICAL_DRAW) || !defined(USE_FAST_DRAW)
   FLASH_ADDR Address;
   ...
   {
  #endif
  #if defined(USE_FAST_DRAW) && defined(USE_FAST_VERTICAL_DRAW)
      WriteFastVerticalImage(&ImageData, StartX, StartY);
  #else
      if(gfx_DisplayOrientation == DISPLAY_HORIZONTAL)
      {
...
//
// Returns:
//    HANDLE_ID Handle Id assigned to the image.
//
///////////////////////////////////////////////////////////////////////////////
HANDLE_ID AdHandle(uint16_t ImageIndex, uint16_t x, uint16_t y)
{
   HANDLE_ID HandleId;
   ...
   if(Ptr != NULL)
   {
      ...
      if(Header.AreaCount > 0)
      {
         ...
         if(gfx_ImageHandle[HandleIndex].Area != NULL)
         {
            ...
            {
               aPtr = &gfx_ImageHandle[HandleIndex].Area[i];
               
               iPtr = &Header.Area[i];
                  
               aPtr->AreaIndex = i;
               aPtr->AreaInfo.x1 = iPtr->x1;
               aPtr->AreaInfo.y1 = iPtr->y1;
               aPtr->AreaInfo.x2 = iPtr->x2;
               aPtr->AreaInfo.y2 = iPtr->y2;
               aPtr->AreaInfo.Type = iPtr->Type;
            }
         }
      }
      else
...
//
///////////////////////////////////////////////////////////////////////////////
int1 HasHandle(HANDLE_ID HandleId)
{
   uint16_t i;
...
//
///////////////////////////////////////////////////////////////////////////////
int1 GetHandle(HANDLE_ID HandleId, HANDLE_STRUCT *Handle)
{
   uint16_t i;
...
void GetCharacter(FONT *Font, char Character, uint8_t *Ptr)
{
   ...
   else
   {
      ...
      flash_ReadData(Address, (uint16_t *)Ptr, FontSize);
   }
}
...
//
// Returns:
//    IMAGE_ERR indicating if an error occurred, see IMAGE_ERR typedef in 
//    gfx_graphics.h for values.  A return value of IMAGE_ERR_OK indicates that 
//    not error occurred and a return value of IMAGE_ERR_DONE indicates that 
...
IMAGE_ERR gfx_LoadImage(uint16_t Count, uint8_t *Data, int1 New)
{
   ...
   if((Error != IMAGE_ERR_OK) || (PixelCount >= Pixels))
   {
      ...
      if(Error == IMAGE_ERR_OK)
         flash_WriteData(HeaderAddress, (uint16_t *)BitmapData, (sizeof(IMAGE) / 2));
...
//
// Returns:
//    FONT_ERR indicating if an error occurred, see FONT_ERR typedef in 
//    gfx_graphics.h for values.  A return value of FONT_ERR_OK indicates no 
//    error occurred and a return value of FONT_ERR_DONE indicates that font was
//    loaded correctly.
//
///////////////////////////////////////////////////////////////////////////////
FONT_ERR gfx_LoadFont(uint16_t Count, uint8_t *Data, int1 New)
{
   ...
   if((Error != FONT_ERR_OK) || (ByteCount >= Bytes))
   {
      ...
      if(Error == FONT_ERR_OK)
         flash_WriteData(HeaderAddress, (uint16_t *)FontData, (sizeof(FONT) / 2));
...
IMAGE_ERR GetImageAddress(uint16_t ImageIndex, FLASH_ADDR *Address)
{
   ...
   wAddress = ((uint32_t)ImageIndex * (IMAGE_BLOCKS * FLASH_ERASE_SIZE));
      
   return(flash_SetAddress(wAddress, Address));
}
...
IMAGE_ERR GetImageHeaderAddress(uint16_t ImageIndex, FLASH_ADDR *Address)
{
   ...
   Error = GetImageAddress(ImageIndex, Address);
...
IMAGE_ERR GetImageHeader(uint16_t ImageIndex, IMAGE_HEADER *Header)
{
   ...
   if(Error == IMAGE_ERR_OK)
   {
      flash_ReadData(Address, (uint16_t *)Header, (sizeof(IMAGE_HEADER) / 2));
      
      if(Header->Index == 0xFFFF)
...
FONT_ERR GetFontHeaderAddress(uint16_t FontIndex, FLASH_ADDR *Address)
{
   ...
   Error = (FONT_ERR)flash_SetAddress((FONT_START_BLOCK * FLASH_ERASE_SIZE) + ((FontIndex - 1) * (FONT_BLOCKS * FLASH_ERASE_SIZE)), Address);
...
FONT_ERR GetFontInfo(uint16_t FontIndex, FONT *FontData)
{
   ...
   if(Error == FONT_ERR_OK)
   {
      flash_ReadData(Address, (uint16_t *)FontData, (sizeof(FONT) / 2));
      
      if(FontData->Header.Index == 0xFFFF)
...
void gfx_TouchTask(void)
{
   ...
   static int16_t XPosition, YPosition;
   ...
   if(touch_Ready())
   {
      ...
      if(gfx_TouchCalibration.Rotated)
      {
         ...
         if(XPosition < 0)
            XPosition = 0;
         else if(XPosition >= GLCD_LINES)
            XPosition = (GLCD_LINES - 1);
         
         if(YPosition < 0)
            YPosition = 0;
         else if(YPosition >= GLCD_PIXELS)
            YPosition = (GLCD_PIXELS - 1);
      }
      else
      {
         ...
         if(XPosition < 0)
            XPosition = 0;
         else if(XPosition >= GLCD_PIXELS)
            XPosition = (GLCD_PIXELS - 1);
            
         if(YPosition < 0)
            YPosition = 0;
         else if(YPosition >= GLCD_LINES)
            YPosition = (GLCD_LINES - 1);
      }
   ...
   if(PreviousState == TOUCHED)
   {
      if(newTouch)
      {
        ...
        #if TOUCH_UPDATE_RATE == 0
         if(g_TouchOnlyUpDown == FALSE)
         {
      ...
      else if(gfx_tick_difference(CurrentTick, PreviousTouchTick) >= (GFX_TICK)TOUCH_RELEASE_RATE)
      {
     ...
     #if TOUCH_UPDATE_RATE != 0
      if((PreviousState == TOUCHED) && (g_TouchOnlyUpDown == FALSE) && (gfx_tick_difference(CurrentTick, PreviousSentTick) >= (GFX_TICK)TOUCH_UPDATE_RATE))
      {
         if(g_TouchFunctionPtr != NULL)
            TouchCallback(TOUCH_TYPE_PRESSING, XPosition, YPosition);
...
void TouchCallback(TOUCH_TYPE Kind, uint16_t RealX, uint16_t RealY)
{
   ...
   int16_t i;
   uint8_t j;
   HANDLE_ID ImageHandle = 0xFFFF;
   uint8_t AreaIndex = 0xFFFF;
   
   if(gfx_HandleCount > 0)
   {
      i = gfx_HandleCount - 1;
      
      while((i >= 0) && (ImageHandle == 0xFFFF))
      {
         ...
         if(Handle->AreaCount > 0)
         {
            ...
            while((j<Handle->AreaCount) && (AreaIndex == 0xFF))
            {
               Area = &Handle->Area[j];
   ...
   if((ImageHandle != 0xFFFF) || (g_TouchOnlyValidAreas == FALSE) || (Kind == TOUCH_TYPE_RELEASED))
...
//
///////////////////////////////////////////////////////////////////////////////
void TouchFunction(HANDLE_ID HandleId, uint8_t AreaIndex, TOUCH_TYPE Kind, uint16_t x, uint16_t y)
{
...
void gfx_InitBacklight(void)
{
   g_CurrentBacklightLevel = g_BacklightLevel = (GFX_BACKLIGHT_START_DUTY * 10);
}
...
void gfx_BacklightTask(void)
{
   ...
   if(gfx_tick_difference(CurrentTick, PreviousTick) >= ((GFX_TICK)GFX_TICKS_PER_SECOND / 1000))
   {
      if(g_CurrentBacklightLevel != g_BacklightLevel)
      {
         ...
         pwm_set_duty_percent(GFX_BACKLIGHT_STREAM, g_CurrentBacklightLevel);
      }
...
void gfx_SerialTask(void)
{
   ...
   static _touchpfunc pTouchfunc = NULL;
   _touchpfunc TouchFunc;
   static int1 pTouchOnlyValidAreas;
   static int1 pTouchOnlyUpDown;
      ...
      switch(Ptr->Cmd)
      {
         ...
         case SERIAL_CMD_DISPLAY_IMAGE:
            if(Ptr->Count == 6)
            {
               ...
               if(HandleId != 0xFFFF)
                  SerialSendData(SERIAL_CMD_DISPLAY_IMAGE, (uint8_t *)&HandleId, 2);
            }
            break;
         ...
         case SERIAL_CMD_DRAW_LINE_AB:
            ...
            break;
         case SERIAL_CMD_SET_TOUCH_CALLBACK:
            if(Ptr->Count > 0)
            {
               if(Ptr->Data[0] == ENABLE_SERIAL_TOUCH)
               {
                  TouchFunc = SerialTouchFunction;
                  
                  if(g_TouchFunctionPtr != TouchFunc)
                  {
                     pTouchfunc = g_TouchFunctionPtr;
                     pTouchOnlyValidAreas = g_TouchOnlyValidAreas;
                     pTouchOnlyUpDown = g_TouchOnlyUpDown;
                     
                     if(Ptr->Count == 1)
                        gfx_SetTouchCallback(SerialTouchFunction, TRUE, TRUE);
                     else
                        gfx_SetTouchCallback(SerialTouchFunction, bit_test(Ptr->Data[1], 0), bit_test(Ptr->Data[1], 1));
                  }
               }
               else
               {
                  if(pTouchfunc != NULL)
                     gfx_SetTouchCallback(pTouchfunc, g_TouchOnlyValidAreas, g_TouchOnlyUpDown);
               }
            }
            break;
         case SERIAL_CMD_IMAGE_HANDLE:
            if(Ptr->Count == 2)
            {
               HandleId = gfx_GetImageHandle(make16(Ptr->Data[1], Ptr->Data[0]));
               
               SerialSendData(SERIAL_CMD_IMAGE_HANDLE, (uint8_t *)&HandleId, 2);
            }
            break;
      }
...
// SerialTouchFunction()
//
// Optional function for sending touches through the Serial Connection.  
// Function is automatically called by gfx_TouchTask() when function has been
// assigned as the touch function with gfx_SetTouchCallback().  Function is 
// automatically assign as the touch function by gfx_SerialTask() when the
// SERIAL_CMD_SET_TOUCH_CALLBACK command is received and the 1st data byte is 
// ENABLE_SERIAL_TOUCH.
//
// Parameters:
//    HandleId - Id of Image that was pressed. -1 if coordinates are not with
//               an Image, can only occur if g_TouchOnlyValidAreas is set to 
//               FALSE.
//
//    AreaIndex - Index of Area that was pressed. -1 if not within an 
//                area of an image, can only occur if g_TouchOnlyValidAreas is 
//                set to FALSE.
//
//    Kind - TOUCH_TYPE specifying the touch type, TOUCH_TYPE_PRESSED,
//           TOUCH_TYPE_RELEASED or TOUCH_TYPE_PRESSING.  TOUCH_TYPE_PRESSING
//           can only occur is g_TouchOnlyUpDown is set to FALSE.
//
//    x - X coordinate that was pressed within area, or display if AreaIndex = 
//        -1.
//
//    y - Y coordinate that was pressed within area, or display if AreaIndex = 
//        -1.
//
///////////////////////////////////////////////////////////////////////////////
void SerialTouchFunction(HANDLE_ID HandleId, uint8_t AreaIndex, TOUCH_TYPE Kind, uint16_t x, uint16_t y)
{
   uint8_t Data[8];
   
   Data[0] = make8(HandleId, 0);
   Data[1] = make8(HandleId, 1);
   Data[2] = AreaIndex;
   Data[3] = Kind;
   Data[4] = make8(x, 0);
   Data[5] = make8(x, 1);
   ...
   SerialSendData(SERIAL_CMD_TOUCH_CALLBACK, Data, 8);
}
...
// SendAckNack()
//
// Optional function for sending a Serial ACK or NACK.  Function is 
