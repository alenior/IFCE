...
#define GOTO_INSTRUCTION(x)   \
   ...
   */
   ...
   #if !defined(E3_DONT_ADD_SERIAL)
   #use rs232(call_putc=usb_cdc_putc, call_getc=usb_cdc_getc, call_kbhit=usb_cdc_kbhit, stream=STREAM_USB)
   #endif
...
// turns off all LEDs, except for the led specified by the parameter 'pin'.
// the led specified by 'pin' is turned on.
// 'pin' can be RED_LED, YELLOW_LED or GREEN_LED
void only_led_on(long pin)
{
   output_low(RED_LED);
...
// wait's until the button has been pressed and then released.
// processor will be deadlocked waiting for button to be pressed.
// preforms very crude debouncing.
