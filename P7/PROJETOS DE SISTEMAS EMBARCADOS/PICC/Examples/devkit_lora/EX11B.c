...
////                                                                   ////
////  This exercise is an example of using the Over-The-Air-Activation ////
////  (OTAA) method to join the LoRaWAN network.  Once joined the      ////
////  program will periodically send the current Temperature, Light    ////
////  sensor and both POT readings to the network.  To function        ////
...
const uint8_t LORAWAN_DEFAULT_APP_EUI[LORAWAN_EUI_SIZE] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF};
const uint8_t LORAWAN_DEFAULT_APP_KEY[LORAWAN_KEY_SIZE] = {0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10,
...
{
   uint8_t EUI[LORAWAN_EUI_SIZE];
   uint8_t i;
   
   lorawan_get_device_eui(EUI);
   
   user_printf(user_putc, "Device EUI: ");
   
   for(i=0;i<LORAWAN_EUI_SIZE;i++)
      user_printf(user_putc, "%02X", EUI[i]);
...
void main(void)
{
   ...
   user_printf(user_putc, "\r\nEX11B.c (LoRaWAN OTAA example) - %s\r\n", getenv("DEVICE"));
