...
//checks if the data (ID) matches any ID stored and EEPROM and returns 1 if it is true
...
void main(void) {
   ...
   while(1) {
      ...
      if(read_4102(code))  //read the code
      {
         ...
         if (wrong_attempts>4) {  // if there are too many attempts, disable for a while
           sprintf(msg,"\r\nToo many attempts!");
           RS485send(msg);
           output_low(RED_LED);
           delay_ms(10000);
