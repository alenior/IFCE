...
// If the define USE_WITH_PC is commented out the example program is built so
// that it communicates with a PC using a serial monitor program using the UART1
// peripheral, and communicates with a slave modbus device using a software 
// RS232 and the external interrupt as the receive interrupt source.  When not
// commented out it communicates with a modbus slave emulator using the UART1
// peripheral and the RDA interrupt as the receive interrupt source.
...
#define MODBUS_SERIAL_BAUD 9600
   ...
   #if defined(__PCD__)
      #define MODBUS_SERIAL_TX_PIN PIN_D0   // Data transmit pin
      #define MODBUS_SERIAL_RX_PIN PIN_E8   // Data receive pin
...
unsigned int8 swap_bits(unsigned int8 c)
{
...
void write_regs()
{
   unsigned int16 reg_array[2] = {0x1111, 0x2222};
